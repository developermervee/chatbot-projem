...
# ------------- STATİK DOSYALAR VE ROOT YÖNLENDİRME -------------
...
# ------------- VERİTABANI TABLOLARI OLUŞTUR -------------
models.Base.metadata.create_all(bind=engine)

# ------------- CORS MİDDLEWARE EKLE -------------
origins = ["*"]
app.add_middleware(
    CORSMiddleware,
...
def update_user_settings(settings: schemas.UserUpdateSettings, current_user: models.User = Depends(get_current_user), db: Session = Depends(get_db)):
...
def forgot_password(request: schemas.PasswordResetRequest, db: Session = Depends(get_db)):
    ...
    if not user:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Bu e-posta adresine sahip bir kullanıcı bulunamadı.")
    password_reset_token = security.create_password_reset_token(email=user.email)
    print("--- ŞİFRE SIFIRLAMA (SİMÜLASYON) ---")
    print(f"Link: reset-password.html?token={password_reset_token}")
    return {"message": "Şifre sıfırlama talimatları gönderildi (simülasyon)."}
...
async def upload_products_from_file(file: UploadFile, current_user: Annotated[models.User, Depends(get_current_user)], db: Session = Depends(get_db)):
    ...
    if file_extension not in ['csv', 'xlsx']:
        raise HTTPException(status_code=400, detail="Lütfen CSV ya da XLSX dosyası yükleyin.")
    try:
        ...
        if not required_columns.issubset(df.columns):
            raise HTTPException(status_code=400, detail="'name' ve 'price' sütunları zorunludur.")
        new_products = []
        ...
        db.commit()
        return {"message": f"{len(new_products)} ürün eklendi."}
    except Exception as e:
        db.rollback()
        raise HTTPException(status_code=500, detail=f"Hata: {e}")

# ------------- SSS (FAQ) ENDPOINTLERİ -------------
...
def create_lead(lead: schemas.LeadCreate, db: Session = Depends(get_db)):
    ...
    if not owner:
        raise HTTPException(status_code=404, detail="Sahip kullanıcı bulunamadı.")
...
def handle_chat(request: schemas.ChatRequest, db: Session = Depends(get_db)):
    ...
    if product_choices:
        ...
        if best_product_match and best_product_match[1] > 80:
            ...
            return {"response": f"'{matched_product.name}' ürünü {matched_product.price} TL. Açıklama: {matched_product.description}"}
    ...
    if "kargo" in user_message or "teslimat" in user_message:
        return {"response": "Kargo 50 TL. 1000 TL ve üzeri ücretsiz."}
    if "merhaba" in user_message or "selam" in user_message:
        return {"response": "Merhaba! Size nasıl yardımcı olabilirim?"}

    return {"response": "Sorunuzu anlayamadım. Bilgilerinizi bırakın, sizinle iletişime geçelim.", "type": "lead_capture"}
